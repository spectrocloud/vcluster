...
func (cmd *PauseCmd) Run(args []string) error {
	...
	err = pauseVCluster(cmd.kubeClient, args[0], cmd.Namespace, cmd.Log)
	if err != nil {
		return err
	}
...
func pauseVCluster(kubeClient *kubernetes.Clientset, name, namespace string, log log.Logger) error {
	...
	found, err := lifecycle.ScaleDownStatefulSet(kubeClient, labelSelector, namespace, log)
	if err != nil {
		return err
	} else if !found {
		found, err = lifecycle.ScaleDownDeployment(kubeClient, labelSelector, namespace, log)
		if err != nil {
			return err
		} else if !found {
		...
		_, err = lifecycle.ScaleDownDeployment(kubeClient, "app=vcluster-api,release="+name, namespace, log)
		if err != nil {
			return err
		}

		// scale down kube controller
		_, err = lifecycle.ScaleDownDeployment(kubeClient, "app=vcluster-controller,release="+name, namespace, log)
		if err != nil {
			return err
		}

		// scale down etcd
		_, err = lifecycle.ScaleDownStatefulSet(kubeClient, "app=vcluster-etcd,release="+name, namespace, log)
		if err != nil {
			return err
		}
	}
