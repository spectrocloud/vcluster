...
func resumeVCluster(kubeClient *kubernetes.Clientset, name, namespace string, log log.Logger) error {
	...
	found, err := lifecycle.ScaleUpStatefulSet(kubeClient, labelSelector, namespace, log)
	if err != nil {
		return err
	} else if !found {
		found, err = lifecycle.ScaleUpDeployment(kubeClient, labelSelector, namespace, log)
		if err != nil {
			return err
		} else if !found {
		...
		_, err = lifecycle.ScaleUpDeployment(kubeClient, "app=vcluster-api,release="+name, namespace, log)
		if err != nil {
			return err
		}

		// scale down kube controller
		_, err = lifecycle.ScaleUpDeployment(kubeClient, "app=vcluster-controller,release="+name, namespace, log)
		if err != nil {
			return err
		}

		// scale down etcd
		_, err = lifecycle.ScaleUpStatefulSet(kubeClient, "app=vcluster-etcd,release="+name, namespace, log)
		if err != nil {
			return err
		}
	}
