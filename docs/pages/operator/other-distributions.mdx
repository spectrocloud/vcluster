...
Behind the scenes a different helm chart will be deployed (`vcluster-k0s`), that holds specific configuration to support k0s. Check the [github repository](https://github.com/spectrocloud/vcluster/tree/main/charts/k0s) for all available chart options.
...
Behind the scenes a different helm chart will be deployed (`vcluster-k8s`), that holds specific configuration to support vanilla k8s. Check the [github repository](https://github.com/spectrocloud/vcluster/tree/main/charts/k8s) for all available chart options.
...
If you want to create a separate chart for the Kubernetes distribution, a good starting point is to copy one of [our distro charts](https://github.com/spectrocloud/vcluster/tree/main/charts) and then modifying it to work with your distribution.
...
For multi binary distributions, vcluster can even create those with a pre-install hook as found in the [k8s chart](https://github.com/spectrocloud/vcluster/tree/main/charts/k8s/templates).
